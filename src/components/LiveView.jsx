...
const MiniFloorCard = ({ floorId, floorData, eventConfig, now, onClick }) => {
    ...
    if (currentDj) {
        ...
        if (diffSec >= 0) {
            ...
            const s = diffSec % 60;
            timeDisplay = h > 0 ? `${h}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}` : `${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`;
        }
    }
            ...
            <div className="space-y-6">
                <section>
                    <div className="bg-surface-background/50 rounded-xl px-4 py-1 space-y-1">
                        ...
                        <Toggle checked={isWakeLockEnabled} onChange={onWakeLockToggle} label="画面のスリープ防止" icon={isWakeLockEnabled ? SunIcon : MoonIcon} />
                    </div>
                    <p className="text-xs text-on-surface-variant mt-2 px-2">※ スリープ防止機能は、ブラウザや端末の設定によっては動作しない場合があります。</p>
                </section>
            </div>
        </BaseModal>
    );
...
const VjDisplay = ({ vjTimetable, eventConfig, now, djEventStatus, suppressAnimation }) => {
    ...
    useEffect(() => {
        if (suppressAnimation) { setVisibleVjContent(currentVjData); setIsVjFadingOut(false); if (vjAnimationTimerRef.current) clearTimeout(vjAnimationTimerRef.current); return; }
        if (!visibleVjContent) { setVisibleVjContent(currentVjData); return; }
        if (currentVjData && visibleVjContent) {
            if (currentVjData.animationKey !== visibleVjContent.animationKey) {
                ...
                vjAnimationTimerRef.current = setTimeout(() => { setVisibleVjContent(currentVjData); setIsVjFadingOut(false); vjAnimationTimerRef.current = null; }, 400);
            } else if (currentVjData.animationKey === visibleVjContent.animationKey && !isVjFadingOut) { setVisibleVjContent(currentVjData); }
        } else if (!currentVjData && visibleVjContent) {
            if (!isVjFadingOut) { setIsVjFadingOut(true); vjAnimationTimerRef.current = setTimeout(() => { setVisibleVjContent(null); setIsVjFadingOut(false); }, 400); }
        }
    }, [currentVjData, visibleVjContent, isVjFadingOut, suppressAnimation]);

    useEffect(() => {
        if (visibleVjContent?.animationKey) { const key = visibleVjContent.animationKey; if (suppressAnimation) { displayedVjKeysRef.current.add(key); } else { const timer = setTimeout(() => { displayedVjKeysRef.current.add(key); }, 500); return () => clearTimeout(timer); } }
...
export const LiveView = ({ timetable, vjTimetable, eventConfig, floors, currentFloorId, setMode, onSelectFloor, loadedUrls, timeOffset, isReadOnly, theme, toggleTheme, eventId, isPreview = false }) => {
    ...
    useEffect(() => {
        ...
        switch (eventStatus) {
            case 'ON_AIR_BLOCK':
                if (currentIndex !== -1) {
                    const dj = schedule[currentIndex]; const nextDj = (currentIndex < schedule.length - 1) ? schedule[currentIndex + 1] : null; const total = (dj.endTimeDate.getTime() - dj.startTimeDate.getTime()) / 1000; const remainingMs = (dj.endTimeDate.getTime() - nowTime); const remainingSeconds = Math.floor(remainingMs / 1000); const visualProgress = (remainingSeconds <= 0 && remainingMs > -1000) ? 100 : (total > 0 ? ((total - (remainingMs / 1000)) / total) * 100 : 0); newDjData = { ...dj, status: 'ON AIR', timeLeft: remainingSeconds, progress: visualProgress, nextDj: nextDj, animationKey: dj.id };
                } else {
                    const upcomingDj = schedule.find(dj => nowTime < dj.startTimeDate.getTime()); if (upcomingDj) { const remainingMs = (upcomingDj.startTimeDate.getTime() - nowTime); const remainingSeconds = Math.ceil(remainingMs / 1000); newDjData = { ...upcomingDj, status: 'UPCOMING', timeLeft: remainingSeconds, progress: 0, nextDj: upcomingDj, animationKey: `upcoming-${upcomingDj.id}` }; } else { newDjData = { id: 'finished', status: 'FINISHED', animationKey: 'finished' }; }
                }
                break;
            case 'UPCOMING':
                { const upcomingDj = schedule.length > 0 ? schedule[0] : null; if (upcomingDj) { const remainingMs = (upcomingDj.startTimeDate.getTime() - nowTime); const remainingSeconds = Math.ceil(remainingMs / 1000); newDjData = { ...upcomingDj, status: 'UPCOMING', timeLeft: remainingSeconds, progress: 0, nextDj: upcomingDj, animationKey: `upcoming-${upcomingDj.id}` }; } else { const remainingMs = (eventStartTimeDate.getTime() - nowTime); const remainingSeconds = Math.ceil(remainingMs / 1000); newDjData = { id: 'empty', status: 'UPCOMING', animationKey: 'empty', timeLeft: remainingSeconds, progress: 0, nextDj: null }; } } break;
            case 'FINISHED': newDjData = { id: 'finished', status: 'FINISHED', animationKey: 'finished' }; break;
            case 'STANDBY': default: newDjData = { id: 'standby', status: 'STANDBY', animationKey: 'standby' }; break;
        }
        setCurrentDjData(newDjData);
    }, [now, schedule, currentlyPlayingIndex, eventStatus, eventStartTimeDate]);

    useEffect(() => {
        if (suppressEntryAnimation) { setVisibleDjContent(currentDjData); setIsDjFadingOut(false); if (djAnimationTimerRef.current) clearTimeout(djAnimationTimerRef.current); return; }
        if (!visibleDjContent) { setVisibleDjContent(currentDjData); return; }
        if (currentDjData && visibleDjContent) {
            if (currentDjData.animationKey !== visibleDjContent.animationKey) { if (djAnimationTimerRef.current) clearTimeout(djAnimationTimerRef.current); setIsDjFadingOut(true); djAnimationTimerRef.current = setTimeout(() => { setVisibleDjContent(currentDjData); setIsDjFadingOut(false); djAnimationTimerRef.current = null; }, 400); } else if (currentDjData.animationKey === visibleDjContent.animationKey && !isDjFadingOut) { setVisibleDjContent(currentDjData); }
        }
    }, [currentDjData, visibleDjContent, isDjFadingOut, suppressEntryAnimation]);

    useEffect(() => { if (visibleDjContent?.animationKey) { const key = visibleDjContent.animationKey; if (suppressEntryAnimation) { displayedDjKeysRef.current.add(key); } else { const timer = setTimeout(() => { displayedDjKeysRef.current.add(key); }, 500); return () => clearTimeout(timer); } } }, [visibleDjContent, suppressEntryAnimation]);

    useEffect(() => {
        const requestWakeLock = async () => { if ('wakeLock' in navigator && isWakeLockEnabled) { try { wakeLockRef.current = await navigator.wakeLock.request('screen'); } catch (err) { console.error(err); } } };
        const releaseWakeLock = () => { if (wakeLockRef.current) { wakeLockRef.current.release(); wakeLockRef.current = null; } };
        requestWakeLock(); const handleVisibilityChange = () => { if (wakeLockRef.current !== null && document.visibilityState === 'visible') requestWakeLock(); };
        document.addEventListener('visibilitychange', handleVisibilityChange); return () => { releaseWakeLock(); document.removeEventListener('visibilitychange', handleVisibilityChange); if (toastTimerRef.current) clearTimeout(toastTimerRef.current); };
    }, [isWakeLockEnabled]);

    const handleWakeLockToggle = () => { const newValue = !isWakeLockEnabled; setIsWakeLockEnabled(newValue); localStorage.setItem('wakeLockEnabled', newValue); showToast(newValue ? 'スリープ防止 ON' : 'スリープ防止 OFF'); };
    ...
    const renderDjContent = (content) => {
        ...
        if (content.status === 'UPCOMING') {
            const dj = content; const eventStartTimeStr = eventStartTimeDate.toTimeString().slice(0, 5); const eventEndTimeStr = eventEndTimeDate.toTimeString().slice(0, 5); const displayColor = schedule.length > 0 && dj.color ? dj.color : '#888888'; const eventTitle = eventConfig.title || "イベント待機中"; const nextDj = dj.nextDj; const nextDjStartTimeStr = nextDj ? nextDj.startTimeDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''; const nextVj = (vjSchedule.length > 0 && (vjEventStatus === 'UPCOMING' || vjEventStatus === 'ON_AIR_BLOCK')) ? vjSchedule[0] : null; const nextVjStartTimeStr = nextVj ? nextVj.startTimeDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
            return (
                <div className="flex flex-col items-center justify-center w-full">
                    ...
                    {(nextDj || (eventConfig.vjFeatureEnabled && hasVjData && nextVj)) && (<div className="w-full max-w-3xl mt-0"><div className="w-full max-w-3xl border-t border-on-surface/10 mb-4" /><div className="flex flex-col items-center justify-center gap-4">{nextDj && (<div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4"><span className="text-lg sm:text-2xl font-bold text-on-surface-variant/50">DJ STANDBY</span><div className="h-6 w-px bg-on-surface/30 hidden sm:block"></div><div className="flex items-center gap-3"><span className="text-base sm:text-lg text-on-surface-variant uppercase font-bold tracking-widest">NEXT DJ</span><span className="text-base sm:text-lg font-semibold truncate max-w-[150px] sm:max-w-xs">{nextDj.name}</span><span className="text-base sm:text-lg text-on-surface-variant font-mono whitespace-nowrap">{nextDjStartTimeStr}~</span></div></div>)}{eventConfig.vjFeatureEnabled && hasVjData && nextVj && (<div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4"><span className="text-lg sm:text-2xl font-bold text-on-surface-variant/50">VJ STANDBY</span><div className="h-6 w-px bg-on-surface/30 hidden sm:block"></div><div className="flex items-center gap-3"><span className="text-base sm:text-lg text-on-surface-variant uppercase font-bold tracking-widest">NEXT VJ</span><span className="text-base sm:text-lg font-semibold truncate max-w-[150px] sm:max-w-xs">{nextVj.name}</span><span className="text-base sm:text-lg text-on-surface-variant font-mono whitespace-nowrap">{nextVjStartTimeStr}~</span></div></div>)}</div></div>)}
                </div>
            );
        }
        if (content.status === 'ON AIR') {
        ...
        if (content.status === 'FINISHED') { return (<div className="text-center animate-fade-in-up"><h1 className="text-4xl sm:text-5xl md:text-7xl font-bold">{isAllFloorsFinished ? 'EVENT FINISHED' : 'FLOOR FINISHED'}</h1></div>); }
        if (content.status === 'STANDBY') { const eventTitle = eventConfig.title || "DJ Timekeeper Pro"; return (<div className="text-center"><h2 className="text-xl sm:text-2xl md:text-3xl text-on-surface-variant font-bold tracking-widest mb-4">STANDBY</h2><h1 className="text-4xl sp:text-5xl sm:text-6xl md:text-7xl font-bold break-words leading-tight">{eventTitle}</h1></div>); }
        return null;
    };
